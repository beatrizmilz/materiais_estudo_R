...
meta() %>%
  ...
  meta_social(
    ...
    description = paste(
      ...
      "Organizado por Beatriz Milz"
    ),
    url = "https://beatrizmilz.github.io/materiais_estudo_R/",
    image = "https://beatrizmilz.github.io/materiais_estudo_R/img/share-card.png",
    image_alt = paste(
...
Caso queira contribuir com algum material, faça uma contribuição via Pull Request no arquivo [neste link](https://github.com/beatrizmilz/materiais_estudo_R/blob/master/dados.csv).
...
```{r}
# data_raw <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1GNmIWjStC1RQXdQjUETclkFVL9QfvDabNi8PP0bFFKk/edit?usp=sharing") %>%  tidyr::drop_na(url)
# 
# readr::write_csv(data_raw, file = "dados.csv")

data_raw <- readr::read_csv("dados.csv")

data <- data_raw %>% 
  tidyr::drop_na(url) %>% 
 dplyr::arrange(tema, idioma)
```
